...
void escribirLog(AppConfig *config, const char *format, ...);
...
int main() {
    ...
    if (dir == NULL) {
        escribirLog(&config, "Error al abrir el directorio: %s\n", config.path_files);
        return EXIT_FAILURE;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_REG) {
            ...
            if (lineas < 0) {
                closedir(dir);
                escribirLog(&config, "Error al contar las líneas del archivo: %s\n", filePath);
                return EXIT_FAILURE;
            }
...
int readConfig(const char *filename, AppConfig *config) {
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if (strcmp(key, "PATH_FILES") == 0) {
            ...
            config->path_files[sizeof(config->path_files) - 1] = '\0'; // Ensure null-termination
        } else if (strcmp(key, "INVENTORY_FILE") == 0) {
            strncpy(config->inventory_file, value, sizeof(config->inventory_file) - 1);
            config->inventory_file[sizeof(config->inventory_file) - 1] = '\0'; // Ensure null-termination
...
void ProcesarFichero(AppConfig *config, infoSucur *sucursal, int *index) {
    ...
    if (csvFile == NULL) {
        escribirLog(config, "Error al abrir el archivo CSV: %s\n", config->inventory_file);
        closedir(dir);
        return;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_REG) {
            ...
            if (archivo == NULL) {
                escribirLog(config, "Error al abrir el archivo de datos: %s\n", filePath);
            ...
            fclose(archivo);
            escribirLog(config, "Archivo procesado: %s\n", filePath);
            sem_post(&config->file_semaphore);
        }
    }
